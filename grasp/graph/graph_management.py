...
class GraphManager:
    def __init__(
        ...
        self.extended_graphs_root_dir: str = str(
            Path(root_dir) / "extended_graphs"
        )
        ...
        self.autoencoder_device: torch.device = torch.device(
            autoencoder_device
        )
    ...
    def run_full_workflow(self) -> None:
        ...
        logger.debug(
            f"Train windows: {self.train_windows}, "
            f"Test windows: {self.test_windows}"
        )
        ...
        for train_window in self.train_windows:
            ...
            gg = GraspGraph(
                ...
                start_time=str(datetime_to_ns_time_US(train_window[0])),
                end_time=str(datetime_to_ns_time_US(train_window[1])),
        ...
        for test_window in self.test_windows:
            ...
            gg = GraspGraph(
                root=self.graphs_root_dir,
                start_time=str(datetime_to_ns_time_US(test_window[0])),
                end_time=str(datetime_to_ns_time_US(test_window[1])),
        ...
        for path in self.graph_storage.train_data_paths:
            logging.info(f"Creating extended graph for training data: {path}")
            logging.info(
                f"Progress "
            ...
            e_graph_path = (
                Path(e_gg.processed_dir) / e_gg.processed_file_names[0]
            )
            self.graph_storage.extended_train_data_paths.append(
                str(e_graph_path)
            )

        for path in self.graph_storage.test_data_paths:
            logging.info(f"Creating extended graph for testing data: {path}")
            logging.info(
                f"Progress "
            ...
            self.graph_storage.extended_test_data_paths.append(
                str(e_graph_path)
            )
