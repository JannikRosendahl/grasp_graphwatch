...
class ExtendedGraspGraph(InMemoryDataset):
    def __init__(
        ...
        autoencoder_device: torch.device = torch.device(  # noqa: B008
            "cuda" if torch.cuda.is_available() else "cpu"  # noqa: B008
        ),
    ) -> None:
        raw_grasp_graph_basename = extract_graph_basename(osp.basename(raw_grasp_graph_path))
        self.raw_grasp_graph_path: str = raw_grasp_graph_path
        self.file_basename = generate_graph_extendedname(raw_grasp_graph_basename)
        self.file_name = self.file_basename + ".pt"

        self.location_autoencoder: LocationEncoder = location_autoencoder
        ...
        logging.info(f"Extended graph loaded from {osp.join(self.processed_dir, self.file_name)}.")  # noqa: LOG015
    ...
    def process(self) -> None:
        ...
        node_locations: list[str] = [str(location) for location in grasp_graph_data.node_location]
        ...
        location_embeddings_tensors = torch.tensor(location_embeddings, dtype=torch.float32)
        ...
        for i in range(num_nodes):
            if bool(mask[i]):
                ...
                if c in cmd_map:
                    ...
                    y_idx[i] = 0.0  # Treat unknown as "nothing observed / same as file or ip"
        ...
        logger.info(
            f"Extended graph processed with {len(graph_data_extended.x)} "  # type: ignore
            f"nodes, {graph_data_extended.edge_index.shape[1]} edges, "  # type: ignore
            f"{graph_data_extended.subject_mask.sum().item()} subjects, "
            f" node feature dimension {graph_data_extended.x.shape[1]}, "  # type: ignore
            f" edge feature dimension {graph_data_extended.edge_attr.shape[1]} ,"  # type: ignore
            f" label dimension {graph_data_extended.y.shape[1]} "  # type: ignore
